"""An in-process broker: the same topic filters, retained messages and delivery as MQTT, without a network.

It joins a producer and a consumer that live in one process (an integration that embeds the IL consumer) and
serves the tests. Semantics kept from MQTT: `+` and `#` filters; a retained message is replayed to a new subscriber
with `retain=True`; a live message reaches subscribers with `retain=False`; an empty retained payload clears the topic.
"""
...
class InProcessTransport:
    def __init__(self) -> None:
        ...
        """Every publish, in order (for a test to look at)."""
    ...
    async def settle(self) -> None:
        """Wait for the asynchronous callbacks started so far (tests)."""
        while self._tasks:
